...
feeding the next step's input. Non-essential steps (metadata, Drive
upload) are wrapped in try/except so a failure there doesn't throw away
a video that already finished rendering.
"""
    ...
    try:
        ...
        if os.environ.get("GOOGLE_REFRESH_TOKEN") and os.environ.get("GDRIVE_FOLDER_ID"):
            ...
            upload_daily_outputs(
                video_path,
                metadata_path or video_path,  # fall back to just re-uploading video if metadata failed
